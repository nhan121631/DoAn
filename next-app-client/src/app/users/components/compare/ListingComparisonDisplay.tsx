...
export default function ListingComparisonDisplay({ listing1, listing2 }: ListingComparisonDisplayProps) {

  // Helper function to check if an amenity is enabled for a given listing
  const isAmenityEnabled = (listingAmenities: Amenity[], amenityLabel: string) => {
  ...
  return (
    <div className="p-6 mx-auto bg-white shadow-md w-250 rounded-xl">
      ...
      <div className="grid  text-gray-700 [grid-template-columns:150px_1fr_1fr] gap-y-4 gap-x-6">
        ...
        <div className="pb-2 text-lg font-bold text-center text-gray-900 border-b md:col-span-1 md:text-left">Listing 2</div>

        {/* Row 1: Price */}
        <div className="font-bold text-gray-900 md:col-span-1">Price</div>
        <div className="md:col-span-1">
          <span className="text-lg font-bold text-green-700">{listing1.price.toLocaleString()} VND/month</span>
        </div>
        <div className="md:col-span-1">
          {listing2.price ? (
            <span className="text-lg font-bold text-green-700">{listing2.price.toLocaleString()} VND/month</span>
          ) : (
        ...
        <div className="font-bold text-gray-900 md:col-span-1">Electricity Rate</div>
        <div className="text-sm md:col-span-1">
          {listing1.electricityRate ? `${listing1.electricityRate.toLocaleString()} VND/kWh` : 'Not specified'}
        </div>
        <div className="text-sm md:col-span-1">
          {listing2.electricityRate ? `${listing2.electricityRate.toLocaleString()} VND/kWh` : 'Not specified'}
        </div>

        {/* Row 5: Water Rate */}
        <div className="font-bold text-gray-900 md:col-span-1">Water Rate</div>
        <div className="text-sm md:col-span-1">
          {listing1.waterRate ? `${listing1.waterRate.toLocaleString()} VND/person/month` : 'Not specified'}
        </div>
        <div className="text-sm md:col-span-1">
          {listing2.waterRate ? `${listing2.waterRate.toLocaleString()} VND/person/month` : 'Not specified'}
        </div>

        {/* Row 6: Description Snippet */}
        <div className="font-bold text-gray-900 md:col-span-1">Description</div>
        <div className="text-sm md:col-span-1 line-clamp-3">{listing1.description}</div>
        ...
        <div className="pt-4 mt-4 text-lg font-bold text-gray-900 border-t md:col-span-3">Amenities</div>

        {allPossibleAmenities.map((amenity, index) => (
          <React.Fragment key={index}>
            <div className="text-sm font-semibold text-gray-800 md:col-span-1">{amenity.label}</div>
            ...
            <div className="text-center md:col-span-1 md:text-left">
              {listing2.id && isAmenityEnabled(listing2.amenities, amenity.label) ? (
                <BsCheckCircleFill className="inline-block text-lg text-green-600" />
              ) : (
                listing2.id ? <FaTimesCircle className="inline-block text-lg text-red-500" /> : <span className="text-gray-500">N/A</span>
              )}
            </div>
          </React.Fragment>
        ))}
