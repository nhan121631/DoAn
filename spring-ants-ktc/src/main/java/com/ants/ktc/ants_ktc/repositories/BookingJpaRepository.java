...
public interface BookingJpaRepository extends JpaRepository<Booking, UUID> {
    ...
    Page<BookingLandlordProjection> findByLandlordIdProjection(@Param("landlordId") UUID landlordId, Pageable pageable);
    ...
    //         "JOIN FETCH r.user ru " +
    //         "JOIN FETCH ru.profile rup " +
    //         "WHERE b.user.id = :userId", countQuery = "SELECT COUNT(b) FROM Booking b WHERE b.user.id = :userId")
    ...
    //         "JOIN FETCH b.user u " +
    //         "JOIN FETCH u.profile up " +
    //         "WHERE r.user.id = :landlordId", countQuery = "SELECT COUNT(b) FROM Booking b WHERE b.room.user.id = :landlordId")
    // Page<Booking> findByRoomUserIdWithDetails(@Param("landlordId") UUID landlordId, Pageable pageable);
}
