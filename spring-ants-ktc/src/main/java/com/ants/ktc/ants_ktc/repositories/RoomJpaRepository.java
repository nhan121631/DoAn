...
public interface RoomJpaRepository extends JpaRepository<Room, UUID> {
    ...
    // Count view of room
    @Query("select coalesce(sum(r.viewCount), 0) from Room r where r.user.id = :userId and r.isRemoved = 0 and r.approval = 1")
    ...
    @Query("""
            ...
            """)
    List<Room> findAvailableRoomsForML(@Param("excludeUserId") UUID excludeUserId);
    ...
    @Query(value = "SELECT r.* " +
            ...
            "AND r.hidden = 0 " +
            "AND r.is_removed = 0 " +
            "AND r.user_id != UNHEX(REPLACE(:excludeUserId, '-', '')) " +
            "AND r.id NOT IN (" +
            "    SELECT f.room_id FROM favorites f " +
            "    WHERE f.user_id = UNHEX(REPLACE(:excludeUserId, '-', ''))" +
            ") " +
            "AND (:userLat IS NULL OR :userLng IS NULL OR a.lat IS NULL OR a.lng IS NULL OR " +
            "    (6371 * acos(cos(radians(:userLat)) * cos(radians(a.lat)) * " +
            "     cos(radians(a.lng) - radians(:userLng)) + " +
            "     sin(radians(:userLat)) * sin(radians(a.lat)))) <= :radiusKm) " +
            "ORDER BY " +
            "CASE " +
            "  WHEN :userLat IS NULL OR :userLng IS NULL OR a.lat IS NULL OR a.lng IS NULL " +
            "  THEN 999999 " +
    ...
    List<Room> findRoomsWithinRadiusForML(
            @Param("userLat") Double userLat,
            @Param("userLng") Double userLng,
            @Param("radiusKm") Double radiusKm,
            @Param("excludeUserId") String excludeUserId);
    ...
    /**
     * Count available rooms within radius for ML statistics
     */
    @Query(value = "SELECT COUNT(*) " +
            ...
            "    (6371 * acos(cos(radians(:userLat)) * cos(radians(a.lat)) * " +
            "     cos(radians(a.lng) - radians(:userLng)) + " +
            "     sin(radians(:userLat)) * sin(radians(a.lat)))) <= :radiusKm)", nativeQuery = true)
    Long countRoomsWithinRadiusForML(
            @Param("userLat") Double userLat,
            @Param("userLng") Double userLng,
            @Param("radiusKm") Double radiusKm,
            @Param("excludeUserId") String excludeUserId);
    ...
    /**
     * Find rooms with price range filter for ML (optimized for price-based
     * filtering)
     */
    @Query("""
            SELECT r FROM Room r
            ...
            ORDER BY ABS(r.price_month - :targetPrice)
            """)
    List<Room> findRoomsByPriceRangeForML(
            @Param("minPrice") Double minPrice,
            @Param("maxPrice") Double maxPrice,
